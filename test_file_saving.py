...
# Create a test file in musicas directory
test_music_file = os.path.join(MEDIA_DIR, f"test_{int(time.time())}_music.txt")
with open(test_music_file, "w") as f:
    f.write("This is a test music file")

print(f"Test music file created: {os.path.exists(test_music_file)}")

# Create a test file in images directory
test_image_file = os.path.join(IMAGES_DIR, f"test_{int(time.time())}_image.jpg")
with open(test_image_file, "w") as f:
    f.write("This is a test image file")

print(f"Test image file created: {os.path.exists(test_image_file)}")
